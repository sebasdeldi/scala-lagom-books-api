...
class WbooksServiceImpl(
  ...
  /**
    ...
    */
  private def entityRef(id: String): EntityRef[WbooksCommand] =
    clusterSharding.entityRefFor(WbooksState.typeKey, id)
  ...
  override def greetingsTopic(): Topic[api.GreetingMessageChanged] =
    TopicProducer.singleStreamWithOffset { fromOffset =>
      persistentEntityRegistry
        .eventStream(WbooksEvent.Tag, fromOffset)
        .map(ev => (convertEvent(ev), ev.offset))
    }

  private def convertEvent(
    helloEvent: EventStreamElement[WbooksEvent]
