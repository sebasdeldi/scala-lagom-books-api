...
/**
  * This provides an event sourced behavior. It has a state, [[WbooksState]], which
  ...
  */
object WbooksBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[WbooksCommand]): Behavior[WbooksCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, WbooksEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[WbooksCommand, WbooksEvent, WbooksState](
        persistenceId = persistenceId,
        emptyState = WbooksState.initial,
...
/**
  ...
  */
case class WbooksState(message: String, timestamp: String) {
  def applyCommand(cmd: WbooksCommand): ReplyEffect[WbooksEvent, WbooksState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: WbooksEvent): WbooksState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[WbooksEvent, WbooksState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[WbooksEvent, WbooksState] =
    Effect
...
object WbooksState {
  ...
  /**
    ...
    */
  def initial: WbooksState = WbooksState("Hola", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[WbooksCommand]("WbooksAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[WbooksState] = Json.format
}

/**
  * This interface defines all the events that the WbooksAggregate supports.
  */
sealed trait WbooksEvent extends AggregateEvent[WbooksEvent] {
  def aggregateTag: AggregateEventTag[WbooksEvent] = WbooksEvent.Tag
}

object WbooksEvent {
  val Tag: AggregateEventTag[WbooksEvent] = AggregateEventTag[WbooksEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends WbooksEvent
...
/**
  ...
  */
trait WbooksCommandSerializable

/**
  * This interface defines all the commands that the WbooksAggregate supports.
  */
sealed trait WbooksCommand
    extends WbooksCommandSerializable
...
/**
  ...
  * application loader.
  */
object WbooksSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[WbooksState],
